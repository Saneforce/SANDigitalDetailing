...
#import "StockistSelCtrl.h"
#import "mCustomerCell.h"

@interface StockistSelCtrl ()
@property (nonatomic, strong) NSArray* ObjStockistList;
@property (nonatomic, strong) NSArray* StockistList;
...
@implementation StockistSelCtrl
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    if(_SetupData.StkActivityNeed==0){_btnActivity.hidden=YES;}
    ...
    NSMutableArray *MkList=[[_ObjStockistList filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"Town_Code contains[c] %@", self.TdayPl.Pl]] mutableCopy];
    self.StockistList = [[MkList sortedArrayUsingDescriptors:sortDescriptors] mutableCopy];
    
    MkList=[[_ObjStockistList filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"Town_Code!= %@", self.TdayPl.Pl]] mutableCopy];
    self.StockistList = [[self.StockistList arrayByAddingObjectsFromArray:[MkList sortedArrayUsingDescriptors:sortDescriptors]] mutableCopy];
    ...
    if([self.meetData.DataSFHQ  isEqual:@""]) [_btnSelHQ setTitle:NSLocalizedString(@"Selectt the Headquaters", @"Select the Headquaters") forState:UIControlStateNormal];
...
{
    return self.StockistList.count;
}
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString* Code=[self.StockistList[indexPath.row] objectForKey:@"Town_Code"];
    ...
    cell.lCustName.text = [self.StockistList[indexPath.row] objectForKey:@"Name"];
    cell.lTownName.text = [self.StockistList[indexPath.row] objectForKey:@"Town_Name"];
    //cell.lCategory.text = [self.StockistList[indexPath.row] objectForKey:@"Category"];
    //cell.lSpeciality.text = [self.StockistList[indexPath.row] objectForKey:@"Specialty"];
    
    NSMutableArray *MkList=[[_MissedEntry.MissDatas filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"CustCode== %@", [self.StockistList[indexPath.row] objectForKey:@"Code"]]] mutableCopy];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(self.meetData.MissedEntry==YES){
        ...
        if ([MkList count]>0){
            ...
            MissDocItem.CustCode=[self.StockistList[indexPath.row] objectForKey:@"Code"];;
            MissDocItem.CustName=[self.StockistList[indexPath.row] objectForKey:@"Name"];;
            MissDocItem.CusType=@"3";
    ...
    self.CustCode=[self.StockistList[indexPath.row] objectForKey:@"Code"];
    self.CustName=[self.StockistList[indexPath.row] objectForKey:@"Name"];
    self.lSelCustName.text = [self.StockistList[indexPath.row] objectForKey:@"Name"];
    self.lSelTownName.text = [self.StockistList[indexPath.row] objectForKey:@"Town_Name"];
    self.lSelAddr.text = [self.StockistList[indexPath.row] objectForKey:@"Addr"];
    self.lSelContact.text = [self.StockistList[indexPath.row] objectForKey:@"Stockiest_Contact"];
   ...
   // _lineChart.backgroundColor=[UIColor colorWithRed:57.0/255 green:67.0/255 blue:96.0/255 alpha:1.0f];
    ...
    [WBService SendServerRequest:@"GET/CusMVst" withParameter:[@{@"CusCode":self.CustCode,@"typ":@"S"} mutableCopy] withImages:nil DataSF:nil
    ...
    self.lSelPhone.text = [self.StockistList[indexPath.row] objectForKey:@"Stockiest_Phone"];
    self.lSelMobile.text = [self.StockistList[indexPath.row] objectForKey:@"Stockiest_Mobile"];
    self.lSelFax.text = [self.StockistList[indexPath.row] objectForKey:@"Stockiest_Fax"];
    self.lSelEmail.text = [self.StockistList[indexPath.row] objectForKey:@"Stockiest_Email"];
    ...
    [WBService SendServerRequest:@"GET/CusLVst" withParameter:[@{@"CusCode":self.CustCode,@"typ":@"S"} mutableCopy] withImages:nil DataSF:nil
                      completion:^(BOOL success, id respData, NSMutableDictionary *DatawithImage){
...
-(IBAction)searchDoctor:(id)sender
{
    ...
    NSString *DataKey=[[NSString alloc] initWithFormat:@"StockistDetails_%@.SANAPP",self.meetData.DataSF];
    self.StockistList =[[[NSUserDefaults standardUserDefaults] objectForKey:DataKey] mutableCopy];
    if([self.searchBox.text isEqualToString:@""]==NO){
        self.StockistList = [self.StockistList filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"Name contains[c] %@", self.searchBox.text]];
    }
    [self.collectionView reloadData];
}
...
-(IBAction)ShowActivityEntry:(id)sender{
    [self performSegueWithIdentifier:@"ShowStkActivity" sender:self];
}
-(IBAction)goDetailEntry:(id)sender{
    long indx=((UIButton*) sender).tag;
    _MissedEntry.SelectedIndex=indx;
    [self performSegueWithIdentifier:@"skipStkPresent" sender:self];
}
...
-(IBAction)goPreparePresentaion:(id)sender{
    [self assignValues];
    [self performSegueWithIdentifier:@"goStkPreDemo" sender:self];
}
...
-(IBAction)showStockistList:(id)sender{
    self.selStkView.hidden=NO;
}
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if ([[segue identifier] isEqualToString:@"ShowStkActivity"]){
        DynamicActivityCtrl *ActivityCTRL=[segue destinationViewController];
        [ActivityCTRL setEMode:@"3,"];
    }
}
@end
